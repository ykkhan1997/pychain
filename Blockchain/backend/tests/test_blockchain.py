...
def test_blockchain():
    ...
    blockchain.chain[0].hash==GENESIS_DATA['hash']
...
def test_is_valid_chain_bad_genesis(blockchain_three_blocks):
    ...
    with pytest.raises(Exception,match='The genesis block must be correct'):
        Blockchain.is_valid_chain(blockchain_three_blocks.chain)
...
def test_is_valid_transaction_chain_duplicate_chain(blockchain_three_blocks):
    
    transaction=Transaction(Wallet(),'recipient',1)
    
    blockchain_three_blocks.add_block([transaction.to_json(),transaction.to_json()])
...
def test_is_valid_transaction_chain_invalid_chain(blockchain_three_blocks):

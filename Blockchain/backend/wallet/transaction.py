...
from backend.config import MINING_REWARDS, MINING_REWARDS_INPUT
...
class Transaction:
    ...
    def is_valid_transaction(transaction):
        ...
        if transaction.input==MINING_REWARDS_INPUT:
            if list(transaction.output.values())!=[MINING_REWARDS]:
    ...
    def reward_transaction(miner_wallet):
        "Generate a reward transaction that award miners"
        output={}
        output[miner_wallet.address]=MINING_REWARDS
        return Transaction(input=MINING_REWARDS_INPUT,output=output)
def main():
    ...
    print(f'\ntransaction_josn.__dict__: {restored_transaction.__dict__}')
